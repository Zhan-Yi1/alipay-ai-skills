...
## 效果说明
使用本工具和友商的相似工具，分别基于支付宝真实对客小程序进行AI化改造，并从5个角度来评估改造效果。
- **Query通过率：** 生成的 Skill 通过完整评测 Query 的比率
- **用户动线完成度：** 用户动线的覆盖完整性、参数证据、动作承接和失败恢复情况
- **源码忠实度：** API、页面、字段、参数、状态和业务规则是否来自真实源码
- **安全与权限边界：** 高风险动作是否避免自动执行，是否出现低风险动作矫枉过正的情况
- **组件交互质量：** 卡片展示、按钮动作、半屏、原页面承接和用户下一步交互是否清晰
  
评估结果如下图：

![动态评测效果对比](./assets/benchmark-comparison.svg)
